...
- `--model` - OpenRouter model id of the form `provider/model-slug` (default: `google/gemini-2.5-flash`, a cheap, video-capable model ~4x cheaper than `gemini-2.5-pro`). Other options: `google/gemini-2.5-flash-lite` (cheapest), `google/gemini-2.5-pro` (highest quality), `anthropic/claude-sonnet-4.5` (strong vision, no video). Pass the full `provider/slug` form — a bare slug yields "Unknown model". Browse all available ids at https://openrouter.ai/models.
